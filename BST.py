...
def already_present(important):
    for key,point in important:
        ...
        pygame.draw.circle(gameDisplay,yellow,[start_node[0]+sum(point),start_node[1]+length*100],20)
        TextSurf_node ,TextRect_node= text_object(str(key.val),mediumText,black)
        
        if len(str(key.val)) == 1:
            gameDisplay.blit(TextSurf_node,(start_node[0]-6+sum(point),start_node[1]-10+length*100))
        if len(str(key.val)) == 2:    
            gameDisplay.blit(TextSurf_node,(start_node[0]-10+sum(point),start_node[1]-10+length*100))
        if len(str(key.val)) == 3:
            gameDisplay.blit(TextSurf_node,(start_node[0]-16+sum(point),start_node[1]-10+length*100))   
...
while not gameExit:
    ...
    TextSurf ,TextRect= text_object("Press Q to Quit",SmallText,retro[(change+1)%3])
    ...
    if pointer >= 0 :
        if pushed:
            ...
            print(f"      {road}    ")
            pygame.draw.circle(gameDisplay,red,[start_node[0]+lead_x,start_node[1]+lead_y],20)
            pygame.draw.line(gameDisplay,yellow,[start_node[0]+lead_x,start_node[1]+lead_y-100],[start_node[0]+lead_x,start_node[1]+lead_y],1)      
